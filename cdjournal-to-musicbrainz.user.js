...
// @description:ja  CDJournal.com のレコードページに「MusicBrainz に投稿」というリンクを追加します（作品名の右側に）。リンクをクリックすると、レコードの詳細で書き込み済み MusicBrainz の Add Release（リリースを投稿）ページが開きます。
// @license         Unlicense
...
const values =
    ...
    .reduce((r, el) => {
        ...
        else if (/^形態/.test(text))       r.type = selIn(el, 'div').textContent.split(' /')[0];
        else if (/^レーベル/.test(text))    r.label = selIn(el, 'div').textContent;
        else if (/^規格品番/.test(text))    r.cat = selIn(el, 'div').textContent;
        else if (/^発売日/.test(text))      r.date = selIn(el, 'div').textContent.split('/');
        return r;
    }, {});
